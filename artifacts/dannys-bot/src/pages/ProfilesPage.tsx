...
const STATUS_META: Record<string, {
  ...
  logged_out:           { label: "Logged Out",           icon: LogOut,      pill: "bg-yellow-50 text-yellow-700 border-yellow-200" },
      ...
      <div className="desktop-card overflow-hidden flex flex-col" style={{ height: "calc(100vh - 178px)" }}>
          ...
          <div className="flex items-center gap-3 px-3 py-1.5 border-b border-border bg-muted/40 text-[12px] font-bold uppercase tracking-wide text-foreground select-none shrink-0">
            ...
            {profColOrder.filter(k => k !== "account" && k !== "ip" && profColVisible[k as keyof typeof DEFAULT_PROFILES_COL_VISIBLE]).map(key => {
              ...
              if (key === "actions") return <div key={key} {...dragProps} style={{ width: profColWidths.actions }} className={`shrink-0 text-center cursor-default select-none ${dragBorder}`}>Actions</div>;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ width: profColWidths.account }} className="shrink-0 min-w-0 flex items-center gap-1">
                    <Link href={`/profiles/${profile.id}`} onClick={(e: React.MouseEvent) => { if (e.ctrlKey || e.metaKey) e.preventDefault(); }} className="min-w-0 -ml-2">
                      <span
                  ...
                  {profColOrder.filter(k => k !== "account" && k !== "ip" && profColVisible[k as keyof typeof DEFAULT_PROFILES_COL_VISIBLE]).map(key => {
                    ...
                    if (key === "aliveFor") {
                      ...
                      let aliveLabel: React.ReactNode = <span>0mins</span>;
                      if (validSince) {
