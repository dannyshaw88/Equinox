...
const CHANGELOG: { version: string; date: string; items: { category: string; text: string; technical?: string[] }[] }[] = [
  ...
  },
  {
    ...
    date: "2026-09-21",
    items: [
      { category: "Fix", text: "Force Emulation no longer emits a Reels API call. The clips home request now runs only when the View Reels tool itself is executed." },
    ],
  },
  {
    version: "1.1.556",
  ...
  },
  {
    ...
    date: "2026-09-21",
    items: [
      { category: "Fix", text: "Check Direct Messages no longer runs the unrelated mobile UsersInfo and notification warm-up requests before loading the DM inbox." },
    ],
  },
  {
    version: "1.1.553",
  ...
  },
  {
    version: "1.1.540",
    date: "2026-09-20",
    items: [{ category: "UI", text: "Aligned the Accounts Manager browse icon with the other action icons and placed Flag Burnt Proxy beside Flag as Locked Account." }],
  },
  {
    version: "1.1.539",
    date: "2026-09-20",
  ...
  },
  {
    ...
    items: [
      {
        category: "Added",
        text: "API Leak Check now has a 5th check: Mobile App Header Set. The old changelog entries about Pigeon session ID, Bloks version ID, and device/Android IDs applied to the API path (which impersonates the real Instagram Android app), not the EB browser — a real Chrome browser should never send those headers at all. This new check verifies the opposite side: that the API path's calls DO carry the full Android header set (X-Pigeon-Session-Id, X-Pigeon-Rawclienttime, X-Bloks-Version-Id, X-IG-Device-ID, X-IG-Android-ID, X-IG-Connection-Speed/-Type, bandwidth headers) since their absence there is the actual bot signal.",
        technical: [
          "Added mobileHeaderResult check to /api/profiles/:id/api-leak-check in instagram.ts — static readiness check against stored device state, since these calls happen server-side (no window to attach CDP to)",
          "Flags missing X-IG-Device-ID/-Android-ID when device state hasn't been populated by account verify yet",
          "Confirms Pigeon session ID, Bloks version ID, and bandwidth headers are always generated by _buildMobileHeaders() regardless of device state",
          "Added mobileHeaders to the ApiLeakCheckResponse checks type in ApiLeakCheck.tsx — renders automatically via the existing generic Object.values(checks) card loop",
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    date: "4 Jul 2026",
    items: [
      { category: "Reverted", text: "Reverted the v1.1.320 session-stop behaviour on DM inbox errors." },
    ],
  },
  {
    version: "1.1.320",
  ...
  },
  {
    ...
    date: "14 Jun 2026",
    items: [
      { category: "New", text: "Evasion Stats Theories: added IP Login Rate Limit theory — Instagram appears to limit how many browser + API login pairs an IP can perform per hour. Each browser + API pair counts as 1 login event. Theory threshold: ~1–2 verifies per 90 min per IP. The likelihood bar is computed live from your flagged accounts that were banned with zero tool activity." },
    ],
  },
  {
    version: "1.0.964",
