...
const FAQ_SECTIONS = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "All data — accounts, settings, stats, logs, device fingerprints, session cookies — is stored locally in a SQLite database beside the installed Equinox executable (for example, C:\\Program Files\\Equinox\\database.db). Browser session data (Chrome profiles) is stored alongside it. Nothing is ever uploaded anywhere. Everything persists across updates, while a fresh install directory starts with a fresh database.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "In Settings → Data & Backup, click Create Backup. This saves a timestamped copy of database.db to the backups folder beside the installed Equinox executable. You can restore any backup from the same page. Automating backups via Windows Task Scheduler is also possible — copy the database.db file from the installation folder on a schedule.",
      },
      {
  ...
  return (
    <AppLayout>
      <div className="max-w-3xl mx-auto pb-12">
        ...
        {/* Quick-Start */}
        <div className="desktop-card p-6 mb-6">
          <h2 className="text-base font-semibold mb-1">Quick-Start Guide</h2>
          <p className="text-xs text-muted-foreground mb-5">Follow these steps in order every time you add a new account.</p>
          <ol className="space-y-5">
            {[
              {
                title: "Add an Account",
                detail: 'Click "Create Account" in the left sidebar. Enter the Instagram username and password. Click Save.',
              },
              {
                title: "Assign a Proxy",
                detail: "Open the account → Settings tab. Paste the proxy in the box. Format: host:port:username:password. Click Save. Do this before anything else — never run an account without a proxy.",
              },
              {
                title: "Verify the Account",
                detail: 'Click the "Verify" button on the account. Wait for the green tick. If a CAPTCHA or checkpoint appears, open the Embedded Browser tab, complete the challenge manually, then click Verify again.',
              },
              {
                title: "Add Follow Sources",
                detail: "Open the account → Follow Tool tab. Under Target Sources, click Add Source. Enter a hashtag (e.g. fitness) or a target Instagram username (e.g. @nike) whose followers you want to follow.",
              },
              {
                title: "Set Your Limits",
                detail: "Still in the Follow Tool, set Users Per Session, Max Per Day, and the delay between follows. If you are unsure, leave the defaults and adjust later.",
              },
              {
              ...
              },
              {
                ...
                detail: "Open the Human Sessions tab and enable it. This simulates natural browsing (stories, likes, notifications) alongside the follow tool and is important for account health.",
              },
              {
                title: "Watch the Dashboard",
                detail: "Go to Dashboard in the left sidebar. The Activity Log shows every follow, skip, block, and session in real time. If something is wrong it will show here.",
              },
            ].map(({ title, detail }, i) => (
