...
function getBackupsDir(): string {
  return path.join(getInstallDataPath(), "backups");
}
...
async function createWindow() {
  ...
  _serverDebugLogPath = path.join(getInstallDataPath(), "equinox-debug.log");
  ...
  const cookiesDir = path.join(getInstallDataPath(), "browser-data");
  let ebIpcPort = 0;
  try {
