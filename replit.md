...
7. API: Express 5
8. DB: SQLite + better-sqlite3 + Drizzle ORM. Development uses `process.cwd()/database.db`; packaged Windows Electron uses `database.db` beside the installed executable.
...
21. `database.db` — SQLite database in the workspace during development; packaged Windows installs create their own copy beside the executable

## Architecture decisions

22. SQLite (not PostgreSQL) — single-file database for portability; development uses `process.cwd()/database.db`, while packaged Windows installs use the install directory
...
54. The DB path uses `DATABASE_PATH` when Electron starts the packaged server. That path is the install directory's `database.db`; without it, development falls back to `process.cwd()/database.db`
